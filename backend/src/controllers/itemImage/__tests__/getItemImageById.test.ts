...
describe("GET /api/items/:itemId/images/:id", () => {
  it("should return status 401 for unauthorized users", async () => {
    const response = await request(app).get(ITEM_IMAGE_URL);
  ...
  it("should return 404 when the image does not exist on the item", async () => {
    mockAuth(prismaMock);
    prismaMock.item.findUnique.mockResolvedValue(mockItem as unknown as Item);
