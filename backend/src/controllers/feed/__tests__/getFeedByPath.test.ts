...
describe("GET /api/feeds/by-path/:path", () => {
  ...
  it("should default to COLLECTION subjectType when not specified", async () => {
    mockAuth(prismaMock);
    prismaMock.feed.findFirst.mockResolvedValue(mockFeed);
    ...
    expect(prismaMock.feed.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ subjectType: "COLLECTION" }),
      }),
    );
  });

  it("should use SINGLE subjectType when specified", async () => {
    mockAuth(prismaMock);
    ...
    expect(prismaMock.feed.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ subjectType: "SINGLE" }),
      }),
    );
  });
});
