...
const ENDPOINT = `${BASE_API_URL}${FEED_ROUTES}/${MOCK_FEED_ID}${COMPONENT_ROUTES}`;
...
describe("GET /api/feeds/:feedId/components", () => {
  ...
  it("should return 200 with components on success", async () => {
    mockAuth(prismaMock);
    prismaMock.feed.findFirst.mockResolvedValue(mockFeed as unknown as Feed);
    prismaMock.component.findMany.mockResolvedValue([mockComponent]);
